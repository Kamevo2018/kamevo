...
if($_SESSION['permission'] == 'admin'){
    ...
    if(isset($_POST['submit'])){
        if(!empty($_POST['publication'])){
            $dataExist = $db->prepare('SELECT * FROM publications WHERE id = ?');
            $dataExist->execute(array(htmlspecialchars($_POST['publication'])));
            $exist = $dataExist->rowCount();
            if($exist > 0){
                $deleteData = $db->prepare('DELETE FROM publications WHERE id = ?');
                $deleteData->execute(array(htmlspecialchars($_POST['publication'])));
                $success = 'La publication '.htmlspecialchars($_POST['publication']).' a été supprimée';
            }else{
...
<div class="mobile-header">
	...
	<div class="container">
		<h3 class='title'>Espace d'administration</h3>
			...
			?>
		 <div class="cedit">
			<p class="label">Supprimer une publication</p><br/><br/>
			<form action="" method="post">
				<label for="publication"><i class="fa fa-group" aria-hidden="true"></i> Id de la publication</label>
