		...
		if ($_FILES['new']['size'] != 0 && $_FILES['new']['error'] == 0){
			...
			if ($uploadOk == 0) {} else {
				if (move_uploaded_file($_FILES["new"]["tmp_name"], $target_file)) {
						if(getUserInfos($_SESSION['id'], "avatar") != 'kavatar.png'){
							unlink("../../img/upload/".getUserInfos($_SESSION['id'], "avatar"));
						}
						$updateAvatar = $db->prepare('UPDATE users SET avatar = ? WHERE id = ?');
						$updateAvatar->execute(array($target_file,$_SESSION['id']));
						$postSuccess = 'Votre image de profil a bien été modifiée';
...
<?php require('../../php/menu.php'); ?>
	<div class="container">
		...
		<form action="" method="post" class="form" enctype="multipart/form-data">
			<label for="new">Nouvelle image de profil</label>
